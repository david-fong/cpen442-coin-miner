...
namespace miner {
   ...
   void ThreadFunc::operator()() {
      coin_blob_[coin_blob_.size()-1-TEAM_MEMBER_ID_BYTES] = (0x100u * params.thread_num) / params.num_threads;
      while (!share.stop) {
   ...
   void ThreadFunc::permute_coin_blob_() {
      for (unsigned i = 0; i < COIN_BLOB_BYTES-TEAM_MEMBER_ID_BYTES; i++) {
         if (i == COIN_BLOB_BYTES-1-TEAM_MEMBER_ID_BYTES) [[unlikely]] {
